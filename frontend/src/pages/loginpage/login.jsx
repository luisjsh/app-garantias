...
function Login({ authInfo, history, setBadNotification }) {
    ...
    if (data) {
      if (data.login) {
        ...
        authInfo({
          ...
          expiration: data.login.tokenExpiration,
        });
  ...
  useEffect(() => {
    if (error) setBadNotification("Error de conexion");
  });
    ...
    <form onSubmit={handleSubmit}>
      ...
      <LoginPage>
        ...
        <CustomInput
          ...
          onChange={formHandler}
        />
        <Span onClick={() => history.push("/changepassword")}>
          ¿Olvidó su contraseña?
        </Span>
        <CustomButton role="primary">Iniciar Sesion</CustomButton>
        <CustomButton onClick={() => history.push("/signup")} role="secundary">
          Registrarse
        </CustomButton>
      </LoginPage>
    </form>
