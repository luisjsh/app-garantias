...
function SignUp({ history, authInfo, setBadNotification }) {
    ...
    (inputProperties) => {
      if (data) {
        data.getUserWithEmail
          ? setInputProperties({
              ...inputProperties,
              emailLoadingVerification: "invalid",
            })
          : setInputProperties({
              ...inputProperties,
              emailLoadingVerification: "valid",
            });
      }
    },
    [data]
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      inputProperties.passwordConfirmBorderColor === "invalid" ||
      inputProperties.emailLoadingVerification === "invalid"
    )
      return alert("Uno de los datos introducidos es incorrecto");
    ...
    try {
      ...
      authInfo({
        token: addedUser.addUser.token,
        expiration: addedUser.addUser.tokenExpiration,
      });
  ...
  const validatePasswordConfirmation = () => {
    item.password === item.passwordConfirmation
      ? setInputProperties({
          ...inputProperties,
          passwordConfirmBorderColor: "valid",
        })
      : setInputProperties({
          ...inputProperties,
          passwordConfirmBorderColor: "invalid",
        });
  };
    ...
    <form onSubmit={handleSubmit}>
      <Page>
        ...
        <CustomInput
          ...
          name="email"
          label="Correo"
          loading={inputProperties.emailLoadingVerification}
          value={item.email}
          handleChange={formHandler}
        />
        <CustomInput
          ...
          handleChange={formHandler}
          pattern=".{8,}"
          comment="Debe ser mayor a 8 caracteres"
        />
        <CustomInput
          name="passwordConfirmation"
          label="Repita su contraseña"
          loading={inputProperties.passwordConfirmBorderColor}
        ...
        <ButtonWrapper>
          ...
          <CustomButton onClick={() => history.push("/login")} role="secundary">
            Iniciar Sesion
          </CustomButton>
        </ButtonWrapper>
      </Page>
